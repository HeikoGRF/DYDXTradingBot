...
def store_cointegration_results(client, df_market_prices):
  ...
  for index, base_market in enumerate(markets[:-1]):
    ...
    for quote_market in markets[index +1:]:
      ...
      # Log pair
      if coint_flag == 1 and half_life <= MAX_HALF_LIFE and half_life > 0 and float(base_candle["usdVolume"]) > 500000 and float(quote_candle["usdVolume"]) > 500000: # base_market not in blacklist and quote_market not in blacklist
